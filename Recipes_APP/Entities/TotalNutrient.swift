...
class TotalNutrient{
    ...
    init(fromJson json: JSON!){
        ...
        let cHOCDFnetJson = json["CHOCDF.net"]
        if !cHOCDFnetJson.isEmpty{
            cHOCDFnet = CA(fromJson: cHOCDFnetJson)
        }
        ...
        let fAMSJson = json["FAMS"]
        if !fAMSJson.isEmpty{
            fAMS = CA(fromJson: fAMSJson)
        }
        let fAPUJson = json["FAPU"]
        if !fAPUJson.isEmpty{
            fAPU = CA(fromJson: fAPUJson)
        }
        ...
        let fATRNJson = json["FATRN"]
        if !fATRNJson.isEmpty{
            fATRN = CA(fromJson: fATRNJson)
        }
        let fEJson = json["FE"]
        if !fEJson.isEmpty{
        ...
        let fOLACJson = json["FOLAC"]
        if !fOLACJson.isEmpty{
            fOLAC = CA(fromJson: fOLACJson)
        }
        let fOLDFEJson = json["FOLDFE"]
        if !fOLDFEJson.isEmpty{
            fOLDFE = CA(fromJson: fOLDFEJson)
        }
        let fOLFDJson = json["FOLFD"]
        if !fOLFDJson.isEmpty{
            fOLFD = CA(fromJson: fOLFDJson)
        }
        let kJson = json["K"]
        if !kJson.isEmpty{
            k = CA(fromJson: kJson)
        ...
        let sUGARJson = json["SUGAR"]
        if !sUGARJson.isEmpty{
            sUGAR = CA(fromJson: sUGARJson)
        }
        let sUGARaddedJson = json["SUGAR.added"]
        if !sUGARaddedJson.isEmpty{
            sUGARadded = CA(fromJson: sUGARaddedJson)
        }
        let sugaralcoholJson = json["Sugar.alcohol"]
        if !sugaralcoholJson.isEmpty{
            sugaralcohol = CA(fromJson: sugaralcoholJson)
        }
        ...
        let wATERJson = json["WATER"]
        if !wATERJson.isEmpty{
            wATER = CA(fromJson: wATERJson)
        }
        let zNJson = json["ZN"]
        if !zNJson.isEmpty{
